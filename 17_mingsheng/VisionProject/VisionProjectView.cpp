...
BEGIN_MESSAGE_MAP(CVisionProjectView, CFormView)
	...
	ON_COMMAND(IDM_ShowIMG, &CVisionProjectView::OnShowimg)
END_MESSAGE_MAP()
...
void CVisionProjectView::OnDraw(CDC* /*pDC*/)
{
	...
	if (pDoc->bUpdateForm)
	{
		...
		//m_gdiAssist.DrawMatToWnd(picWnd->GetSafeHwnd(), scPTGrab2::m_srcMat, Cvgdi::DM_RESIZESRCTOFIT);
		////////////////////m_gdiAssist.DrawMatToWnd(picWnd->GetSafeHwnd(), scPTGrab2::showImage, Cvgdi::DM_RESIZESRCTOFIT);
		//m_gdiAssist.DrawMatToWnd(picWnd->GetSafeHwnd(), scPTGrab2::showImage, Cvgdi::DM_RESIZESRCTOFIT);
		//UpdateWindowInfo();
		...
		StrStateInfo.Format(L"错误数量: %d\r\n触发时间：%d\r\n开始时间：%ld\r\n结束时间：%d\r\n采集次数：%d",
			pDoc->m_scPTGrab2->bErTr,
			pDoc->m_scPTGrab2->dwUsed,
			pDoc->m_scPTGrab2->dwStart,
			GetTickCount(),
			pDoc->m_scPTGrab2->iIndexImage);
		m_CEdit_State.SetWindowText(StrStateInfo);
...
void CVisionProjectView::OnShowimg()
{
	// TODO:  在此添加命令处理程序代码
	if (bShowIMG == true)
	{
