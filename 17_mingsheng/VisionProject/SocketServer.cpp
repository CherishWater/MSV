...
void CSocketServer::OnReceive(int nErrorCode)
{
	...
	*/
	if (strText=="data$")
	{
		if (TDetectData::bTrigger==true)
		{
			WaitForSingleObject(TDetectData::eImageProcessed, INFINITE);
			SendInfo2Client(TDetectData::sendData);
			ResetEvent(TDetectData::eImageProcessed);
		}
		else
		{
			SendInfo2Client(L"1;0;0.0;0.0;0.0;$");
		}
		
		TDetectData::sendData = "1;0;0.0;0.0;0.0;$";
		TDetectData::bTrigger == false;
	}
