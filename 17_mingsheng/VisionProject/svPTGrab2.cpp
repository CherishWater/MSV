...
Mat         scPTGrab2::showImage;
vector<detInfo>	   scPTGrab2::detectInfo;
...
void scPTGrab2::PTImageEventCallback(Image *pImage, const void *pCallbackData)
{
	...
	if (ePTGrab == pThis->mLhreadStatus) // 连续采集
	{
		if (pThis->m_bIsGrabbing)
		{
			...
			//Size roiSize = Size(708, 452);
			//Rect roiRect = Rect(50, 78, roiSize.width, roiSize.height);
			m_srcMat=image.clone();
			//cvtColor(image, m_srcMat, CV_GRAY2RGB);
			//cvtColor(image, m_srcMat, CV_GRAY2RGB);
			
			//CLOG::Out0(L"grab", L"hello %d", TDetectData::iIndex);
			TDetectData::bTrigger = true;
...
//	GetLocalTime(&sysTime);
	...
	//strPartLime.Format(L"[%2d-%2d %2d:%2d:%2d_%3d]",sysTime.wMonth, sysTime.wDay,sysTime.wHour, sysTime.wMinute, sysTime.wSecond,sysTime.wMilliseconds);
	/*
	t3 = sysTime.wMilliseconds;
	t4 = sysTime.wSecond;
...
//	CLOG::Out0(L"grab1", L"%d\n", time1);
	...
	zjlMedicalBagIdentify.imDetect(scPTGrab2::m_srcMat);
	scPTGrab2::showImage = zjlMedicalBagIdentify.m_imResult;

	//CLOG::Out0(L"process ", L"%d", TDetectData::iIndex);
	//CString str;
	//str.Format(L"d%", iIndexImage);
	//string a=str.GetBuffer();
	//imwrite(a,scPTGrab2::m_srcMat);
	detectInfo.clear();
	for (int i = 0; i < zjlMedicalBagIdentify.m_nBagNumber; i++)
	{
		temDetInfo.iIndex = i;
		temDetInfo.itype = 0;
		temDetInfo.x = zjlMedicalBagIdentify.m_vtSendInfo[i].center.x;
		temDetInfo.y = zjlMedicalBagIdentify.m_vtSendInfo[i].center.y;
		temDetInfo.rz = zjlMedicalBagIdentify.m_vtSendInfo[i].angle;

		detectInfo.push_back(temDetInfo);
		detectInfoUI.push_back(temDetInfo);
		//TDetectData::detectInfoSend.push_back(temDetInfo);
	}
	...
	TDetectData::sendData = sendInfo;
	SetEvent(TDetectData::eImageProcessed);
//	pNet->Send(sendInfo);

//	CLOG::Out0(L"send", L"%d", TDetectData::iIndex);
	...
	else
	{
			...
			tt = (getTickCount() - tLast) / TickFrequency;
			...
			{
				//pNet->m_ServerEx.SendInfo2Client(L"Failed$");
